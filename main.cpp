...
int main() {
    ...
    ThreadPool pool(minNum, maxNum, queueCap);
...
#if 0 //测试异常任务在其他测试时执行不合适，会影响其他测试,需要单独测试
    // 4. 测试3：异常任务执行
    std::cout << "\n===== 测试：异常任务执行 =====" << std::endl;
    int* errNum = new int(-1);
    pool.addTask(exceptionTaskFunc, errNum);
    // 延长等待时间，让异常任务执行完成，且其他任务至少执行一部分
    std::this_thread::sleep_for(std::chrono::seconds(3)); 
    // 预期busyNum：maxNum-1（5-1=4），但允许±1（3~5）
    checkPoolStatus(pool, minNum, maxNum-1, maxNum, "异常任务执行后");
#endif
